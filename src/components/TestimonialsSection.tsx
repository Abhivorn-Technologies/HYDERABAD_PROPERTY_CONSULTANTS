...
const TestimonialsSection = () => {
  ...
  return (
    <section id="testimonials" className="section-padding bg-primary relative overflow-hidden" ref={ref}>
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_hsla(43,56%,52%,0.05)_0%,transparent_50%)]" />
      <div className="container mx-auto relative z-10">
        ...
        >
          <div className="glass-dark rounded-3xl p-8 md:p-12 text-center relative">
            <Quote className="text-secondary/20 mx-auto mb-6 animate-float" size={48} />
            <p className="text-primary-foreground/90 text-lg md:text-xl leading-relaxed mb-8 italic font-light">
              "{testimonials[current].review}"
            </p>
            <div className="mb-2">
              <h4 className="text-secondary font-heading font-bold text-xl">{testimonials[current].name}</h4>
              <p className="text-primary-foreground/50 text-sm">{testimonials[current].location}</p>
            </div>
          </div>
          ...
          <div className="flex justify-center gap-4 mt-8">
            <button
              onClick={prev}
              className="w-12 h-12 rounded-full border border-secondary/30 flex items-center justify-center text-secondary hover:bg-secondary/10 transition-colors"
            >
              <ChevronLeft size={20} />
            </button>
