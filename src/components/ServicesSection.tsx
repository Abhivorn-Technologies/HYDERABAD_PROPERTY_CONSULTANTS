      ...
      <div className="container mx-auto relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, i) => (
            <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: i * 0.1 }} className="group glass-dark rounded-2xl p-8 hover:bg-secondary/10 transition-all duration-500 hover:-translate-y-2">
              ...
              <h3 className="text-xl font-heading font-semibold text-primary-foreground mb-3">{service.title}</h3>
              <p className="text-primary-foreground/60 leading-relaxed text-sm">{service.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
